...
def test_validate_extracted_payload_invalid_text_type_fails():
    payload = _valid_payload()
    payload["entity_information"]["name"] = 123
...
def test_validate_extracted_payload_invalid_numeric_type_fails():
    payload = _valid_payload()
    payload["financial_risk_profile"]["components"]["liquidity_adjustment_notches"] = "bad-int"

    with pytest.raises(ValidationError):
        MODULE.validate_extracted_payload(payload)


def test_validate_extracted_payload_invalid_date_type_fails():
    payload = _valid_payload()
    payload["rating_date"] = "2026-02-25"
